...
def test_tree_verification_defers_waiting_prefills():
    scheduler = create_scheduler(max_num_seqs=2, num_speculative_tokens=2)
    scheduler.tree_verification_requires_decode_only = True
    running_req, waiting_req = create_requests(num_requests=2, num_tokens=1)
    scheduler.add_request(running_req)
    ...
    tree = DraftTokenTree.from_draft_tokens([1, 2])
    scheduler.update_draft_token_ids(
        TreeDraftTokenIds([running_req.request_id], [tree])
    )
    scheduler.add_request(waiting_req)

    output = scheduler.schedule()

    assert running_req.request_id in output.scheduled_spec_decode_token_trees
    assert waiting_req.request_id not in output.num_scheduled_tokens
    assert list(scheduler.waiting) == [waiting_req]
