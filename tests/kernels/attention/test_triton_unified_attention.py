...
def test_triton_unified_attn_tree_mask() -> None:
    """Tree queries must see cached context and ancestors, but not siblings."""
    ...
    # Two roots, with three descendants split between them:
    # 0 -> {1, 2}, 3 -> {4}.
    ...
    softmax_segm_output = torch.empty(
        (0, num_heads, num_par_softmax_segments, head_size),
        dtype=torch.float32,
    )
    softmax_segm_max = torch.empty(
        (0, num_heads, num_par_softmax_segments),
        dtype=torch.float32,
    )
    ...
    keys = key_cache[0, :kv_len]
    values = value_cache[0, :kv_len]
    scores = torch.einsum("qhd,khd->hqk", query * scale, keys).float()
    allowed = torch.cat(
        (
            torch.ones(query_len, context_len, dtype=torch.bool),
            tree_attn_mask,
        ),
        dim=1,
    )
    ...
    reference = torch.einsum("hqk,khd->qhd", weights, values)

    torch.testing.assert_close(output, reference, atol=1.5e-2, rtol=1e-2)


@torch.inference_mode()
def test_triton_unified_attn_gqa6_rows_match_single_query_bitwise() -> None:
    """GQA-6 tree and packed-prefill rows must match one-query decoding."""
    ...
    tree_value_cache = torch.randn_like(tree_key_cache)
    ...
    for row in range(query_len):
        if row < 2:
            tree_k = tree_key_cache[:, : context_len + row + 1].clone()
            tree_v = tree_value_cache[:, : context_len + row + 1].clone()
        else:
            tree_k = torch.cat(
                (
                    tree_key_cache[:, : context_len + 1],
                    tree_key_cache[:, context_len + row : context_len + row + 1],
                ),
                dim=1,
            )
            tree_v = torch.cat(
                (
                    tree_value_cache[:, : context_len + 1],
                    tree_value_cache[
                        :, context_len + row : context_len + row + 1
                    ],
                ),
                dim=1,
            )
        tree_reference_k = torch.zeros(
            1,
            ordinary_block_size,
            num_kv_heads,
            head_size,
        ...
        tree_reference_v = torch.zeros_like(tree_reference_k)
        tree_reference_k[:, : tree_k.shape[1]].copy_(tree_k)
        tree_reference_v[:, : tree_v.shape[1]].copy_(tree_v)
        tree_reference = run_attention(
            query[row : row + 1],
            tree_reference_k,
            tree_reference_v,
            tree_k.shape[1],
        ...
        assert torch.equal(tree_output[row], tree_reference)

        linear_kv_len = context_len + row + 1
        linear_reference_k = torch.zeros_like(tree_reference_k)
        linear_reference_v = torch.zeros_like(tree_reference_v)
        linear_reference_k[:, :linear_kv_len].copy_(
            tree_key_cache[:, :linear_kv_len]
        )
        linear_reference_v[:, :linear_kv_len].copy_(
            tree_value_cache[:, :linear_kv_len]
        )
        linear_reference = run_attention(
            query[row : row + 1],
            linear_reference_k,
            linear_reference_v,
            linear_kv_len,
        ...
        assert torch.equal(linear_output[row], linear_reference)
