...
def _minimal_residual_tree_spec_config() -> SpeculativeConfig:
    ...
    for name, value in values.items():
        object.__setattr__(spec_config, name, value)
    return spec_config


def test_residual_tree_accepts_target_model_eager_execution():
    VllmConfig._verify_residual_tree_execution_mode(
        _minimal_residual_tree_spec_config(),
        SimpleNamespace(enforce_eager=True),
    )


def test_residual_tree_rejects_compiled_target_execution():
    with pytest.raises(ValueError, match="requires target model eager execution"):
        VllmConfig._verify_residual_tree_execution_mode(
            _minimal_residual_tree_spec_config(),
            SimpleNamespace(enforce_eager=False),
        )


def test_residual_tree_disables_automatic_async_scheduling():
    cfg = VllmConfig(
        scheduler_config=SchedulerConfig(
            max_model_len=128,
            is_encoder_decoder=False,
        ...
        speculative_config=_minimal_residual_tree_spec_config(),
    )

    assert cfg.scheduler_config.async_scheduling is False


def test_residual_tree_rejects_explicit_async_scheduling():
    with pytest.raises(ValueError, match="not compatible with residual-tree"):
        VllmConfig(
            scheduler_config=SchedulerConfig(
                max_model_len=128,
                is_encoder_decoder=False,
...
def _validated_residual_tree_spec_config(monkeypatch, **overrides):
    # Isolate Pydantic field/model validation from draft-checkpoint loading.
    monkeypatch.setattr(SpeculativeConfig, "__post_init__", lambda self: self)
    values = {
        "method": "eagle",
        "num_speculative_tokens": 2,
        "residual_tree": True,
        "residual_tree_active_heads": [0, 2],
        "residual_tree_head_lambdas": [0.5, 0.3, 0.1],
    }
    values.update(overrides)
    return SpeculativeConfig(**values)


def test_residual_tree_active_heads_accept_ordered_subset(monkeypatch):
    config = _validated_residual_tree_spec_config(monkeypatch)

    assert config.residual_tree_active_heads == [0, 2]


def test_residual_tree_accepts_hybrid_union_candidate_selection(monkeypatch):
...
@pytest.mark.parametrize(
    ...
    [
        ([], "at least 1 item"),
        ([0, 0], "strictly increasing and unique"),
        ([1, 0], "strictly increasing and unique"),
        ([-1], "greater than or equal to 0"),
        ([0, 1.0], "valid integer"),
        ([False], "valid integer"),
        ([0, 3], "without a corresponding lambda"),
    ],
)
def test_residual_tree_active_heads_reject_invalid_config(
    monkeypatch,
    active_heads,
    message,
):
    with pytest.raises(ValidationError, match=message):
        _validated_residual_tree_spec_config(
            monkeypatch,
            residual_tree_active_heads=active_heads,
        )


def _validated_stock_top2_spec_config(monkeypatch, **overrides):
    # Isolate Pydantic field/model validation from draft-checkpoint loading.
    monkeypatch.setattr(SpeculativeConfig, "__post_init__", lambda self: self)
    values = {
        "method": "eagle3",
...
def _validated_stock_dynamic_spec_config(monkeypatch, **overrides):
    # Isolate Pydantic field/model validation from draft-checkpoint loading.
    monkeypatch.setattr(SpeculativeConfig, "__post_init__", lambda self: self)
    values = {
        "method": "eagle3",
        ...
        "residual_tree_candidate_selection": "stock_top9_dynamic",
        "residual_tree_scorer_top_k": 9,
...
def test_stock_top9_dynamic_config_requires_matching_width(monkeypatch):
    config = _validated_stock_dynamic_spec_config(monkeypatch)
    assert config.residual_tree_scorer_top_k == 9

    with pytest.raises(ValidationError, match="requires scorer_top_k=9"):
        _validated_stock_dynamic_spec_config(
            monkeypatch,
...
@pytest.mark.parametrize(
    ("num_speculative_tokens", "max_depth"),
    [(1, 1), (2, None), (3, 1), (7, 2), (15, 3)],
)
def test_stock_top2_config_rejects_infeasible_tree_shapes(
    monkeypatch,
    num_speculative_tokens,
    max_depth,
):
    with pytest.raises(ValidationError, match="fit the configured binary-tree depth"):
        _validated_stock_top2_spec_config(
            monkeypatch,
...
def test_stock_top2_config_accepts_pruned_binary_tree_shapes(
    monkeypatch,
    num_speculative_tokens,
    max_depth,
):
    config = _validated_stock_top2_spec_config(
        monkeypatch,
        num_speculative_tokens=num_speculative_tokens,
        residual_tree_max_depth=max_depth,
    )
    assert config.num_speculative_tokens == num_speculative_tokens
    assert config.residual_tree_max_depth == max_depth


@pytest.mark.parametrize(
    "residual_options",
    [
        {"residual_tree_adapter_config": "/tmp/adapter.json"},
        {
            "residual_tree_adapter_config": "/tmp/adapter.json",
            "residual_tree_adapter_checkpoint": "/tmp/adapter.pt",
        },
        {"residual_tree_head_lambdas": [1.0, 0.5]},
        {"residual_tree_active_heads": [0, 1]},
        {"residual_tree_head_prior_probabilities": [0.4, 0.4, 0.2]},
        {"residual_tree_greedy_scorer_path": "/tmp/scorer.json"},
    ],
)
def test_stock_top2_config_rejects_residual_head_options(
    monkeypatch,
    residual_options,
):
    with pytest.raises(ValidationError, match="must not load or configure"):
        _validated_stock_top2_spec_config(
            monkeypatch,
            **residual_options,
        )
...
def test_stock_top2_config_rejects_trace(monkeypatch):
    with pytest.raises(ValidationError, match="stock_top2 must not"):
        _validated_stock_top2_spec_config(
            monkeypatch,
            residual_tree_trace_path="/tmp/stock-top2-trace.jsonl",
        )


def test_stock_top2_config_accepts_only_eagle3_and_lambda_q(monkeypatch):
    with pytest.raises(ValidationError, match="requires method='eagle3'"):
        _validated_stock_top2_spec_config(monkeypatch, method="eagle")
    with pytest.raises(ValidationError, match="only lambda_q"):
        _validated_stock_top2_spec_config(
            monkeypatch,
            residual_tree_scorer_mode="uniform",
        )


def test_stock_top2_config_requires_best_first_batched_drafting(monkeypatch):
    with pytest.raises(ValidationError, match="requires best_first"):
        _validated_stock_top2_spec_config(
            monkeypatch,
            residual_tree_tree_policy="breadth_first",
        )
    with pytest.raises(ValidationError, match="requires batched tree drafting"):
        _validated_stock_top2_spec_config(
            monkeypatch,
            residual_tree_batch_drafting=False,
        )


def test_stock_top2_config_rejects_residual_novelty_gating(monkeypatch):
    with pytest.raises(ValidationError, match="novelty"):
        _validated_stock_top2_spec_config(
            monkeypatch,
            residual_tree_min_novel_probability_ratio=0.5,
        )


def test_stock_top2_config_requires_greedy_draft_sampling(monkeypatch):
    with pytest.raises(ValidationError, match="requires greedy draft sampling"):
        _validated_stock_top2_spec_config(
            monkeypatch,
...
def test_stock_top2_candidate_requires_tree_mode(monkeypatch):
    with pytest.raises(ValidationError, match="require residual_tree=True"):
        _validated_stock_top2_spec_config(
            monkeypatch,
            residual_tree=False,
        )
