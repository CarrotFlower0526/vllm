...
class TritonAttentionImpl(AttentionImpl):
    ...
    def forward(
        ...
        tree_attn_mask = attn_metadata.tree_attn_mask
        if tree_attn_mask is not None:
            if self.alibi_slopes is not None:
                raise NotImplementedError(
                    "SpecInfer tree attention with ALiBi is not supported"
                )
            if self.sliding_window[0] >= 0 or self.chunk_lookback > -1:
                raise NotImplementedError(
                    "SpecInfer tree attention with sliding-window or chunked "
