...
@triton.jit
def kernel_tree_single_tail_compact(
    output_ptr,
    query_ptr,
    key_cache_ptr,
    value_cache_ptr,
    ...
    tree_attn_mask_stride: tl.int64,
    tree_attn_mask_stride_1: tl.constexpr,
    ...
    HEAD_SIZE: tl.constexpr,
    HEAD_SIZE_PADDED: tl.constexpr,
    HEAD_LANES: tl.constexpr,
    TREE_MASK_WIDTH: tl.constexpr,
    TREE_MASK_LOGICAL_WIDTH: tl.constexpr,
    num_seqs: tl.int32,
):
    """Run one tree node per program in ordinary one-token KV order.

    Complete prefix tiles stay contiguous. Only the final partial tile is
    compacted to prefix-tail + ancestors + self, so masked siblings cannot
    move self across a tile boundary and alter BF16 online-softmax rounding.
    """
    q_block_global_idx = tl.program_id(0)
    kv_head_idx = tl.program_id(1)
    (
    ...
    offs_h = tl.arange(0, HEAD_LANES)
    offs_d = tl.arange(0, HEAD_SIZE_PADDED)
    offs_t = tl.arange(0, TILE_SIZE)
    query_head = kv_head_idx * num_queries_per_kv + offs_h
    head_mask = (offs_h < num_queries_per_kv) & (query_head < num_query_heads)
    dim_mask = offs_d < HEAD_SIZE
    query_row = cur_batch_in_all_start_index + query_pos
    q_offset = (
        query_row * query_stride_0
        + query_head[:, None] * query_stride_1
        + offs_d[None, :]
    )
    Q = tl.load(
        query_ptr + q_offset,
    ...
    tree_cols = tl.arange(0, TREE_MASK_WIDTH)
    tree_allowed = tl.load(
        tree_attn_mask_ptr
        + query_row.to(tl.int64) * tree_attn_mask_stride
        + tree_cols.to(tl.int64) * tree_attn_mask_stride_1,
        mask=tree_cols < TREE_MASK_LOGICAL_WIDTH,
        other=0,
    ).to(tl.int32)
    tree_ranks = tl.cumsum(tree_allowed, axis=0) - 1
    path_len = tl.sum(tree_allowed, axis=0)

    M = tl.full([HEAD_LANES], float("-inf"), dtype=tl.float32)
    L = tl.full([HEAD_LANES], 1.0, dtype=tl.float32)
    acc = tl.zeros([HEAD_LANES, HEAD_SIZE_PADDED], dtype=tl.float32)
    context_len = seq_len - cur_batch_query_len
    block_table_offset = seq_idx * block_table_stride
    full_prefix_tiles = context_len // TILE_SIZE
    for j in range(0, full_prefix_tiles):
        seq_offset = j * TILE_SIZE + offs_t
        physical_block_idx = tl.load(
            block_tables_ptr + block_table_offset + seq_offset // BLOCK_SIZE
        ).to(tl.int64)
        ...
        S = scale * tl.dot(Q, K)
        S = tl.where(head_mask[:, None], S, float("-inf"))
        M, L, P, alpha = softmax_step(S, M, L)
        acc = acc * alpha[:, None]
        acc += tl.dot(P.to(V.dtype), V)

    prefix_remainder = context_len - full_prefix_tiles * TILE_SIZE
    logical_tail_tiles = cdiv_fn(prefix_remainder + path_len, TILE_SIZE)
    for tail_j in range(0, logical_tail_tiles):
        logical_col = tail_j * TILE_SIZE + offs_t
        logical_rank = logical_col - prefix_remainder
        matches = (tree_allowed[:, None] != 0) & (
            tree_ranks[:, None] == logical_rank[None, :]
        )
        selected_tree_col = tl.sum(
            tl.where(matches, tree_cols[:, None], 0), axis=0
        )
        physical_tail_col = tl.where(
            logical_col < prefix_remainder,
            logical_col,
            prefix_remainder + selected_tree_col,
        )
        tile_mask = logical_col < prefix_remainder + path_len
        tail_start = full_prefix_tiles * TILE_SIZE
        seq_offset = tail_start + physical_tail_col
        physical_block_idx = tl.load(
            block_tables_ptr + block_table_offset + seq_offset // BLOCK_SIZE
        ).to(tl.int64)
        k_offset = (
        ...
        S = scale * tl.dot(Q, K)
        S = tl.where(
            head_mask[:, None] & tile_mask[None, :], S, float("-inf")
        )
        M, L, P, alpha = softmax_step(S, M, L)
        acc = acc * alpha[:, None]
        acc += tl.dot(P.to(V.dtype), V)
    ...
    acc = acc / L[:, None]
    output_offset = (
        query_row * output_stride_0
        + query_head[:, None] * output_stride_1
        + offs_d[None, :]
    )
    tl.store(
        output_ptr + output_offset,
...
def unified_attention(
    ...
    use_tree_attn_mask = tree_attn_mask is not None
    tree_attn_mask_stride = tree_attn_mask.stride(0) if use_tree_attn_mask else 0
    tree_attn_mask_stride_1 = (
        tree_attn_mask.stride(1) if use_tree_attn_mask else 0
    )
    tree_attn_mask_ptr = tree_attn_mask if use_tree_attn_mask else q
    ...
    if use_tree_attn_mask:
        use_tree_single_tail_compact = (
        ...
        if not use_tree_single_tail_compact:
            raise NotImplementedError(
                "tree attention requires the BF16 2D causal single-tail kernel"
            )
        tree_num_q_blocks = q.shape[0] + num_seqs - 1
        kernel_tree_single_tail_compact[(tree_num_q_blocks, num_kv_heads)](
            output_ptr=out,
            query_ptr=q,
            key_cache_ptr=k,
            value_cache_ptr=v,
            ...
            tree_attn_mask_ptr=tree_attn_mask_ptr,
            ...
            tree_attn_mask_stride=tree_attn_mask_stride,
            tree_attn_mask_stride_1=tree_attn_mask_stride_1,
            ...
            HEAD_LANES=triton.next_power_of_2(num_queries_per_kv),
            TREE_MASK_WIDTH=triton.next_power_of_2(tree_attn_mask.shape[1]),
            TREE_MASK_LOGICAL_WIDTH=tree_attn_mask.shape[1],
            num_seqs=num_seqs,
            num_warps=4,
            num_stages=2,
        )
