...
class CommonAttentionMetadata:
    ...
    """Optional [num_actual_tokens, max_query_len] bool mask for tree
    speculative decoding. Rows are flattened query tokens; columns are local
    query-token KV positions within that request. Prompt/context KV is always
    visible through the normal backend mask."""
    tree_parent_local_indices_cpu: torch.Tensor | None = None
    """Optional CPU [num_actual_tokens] int32 tensor for tree speculative
    decoding. Each entry is the parent local query index for that flattened
    query row, or -1 for a root/non-tree row."""
    tree_node_depths_cpu: torch.Tensor | None = None
    """Optional CPU [num_actual_tokens] int32 tree depth for each flattened query
    row. Non-tree rows use their local linear query position."""
    ...
    def unpadded(
        ...
        return CommonAttentionMetadata(
            ...
            tree_attn_mask=self.tree_attn_mask[:num_actual_tokens]
            if self.tree_attn_mask is not None
            else None,
            tree_parent_local_indices_cpu=self.tree_parent_local_indices_cpu[
                :num_actual_tokens
            ]
            if self.tree_parent_local_indices_cpu is not None
            else None,
            tree_node_depths_cpu=self.tree_node_depths_cpu[:num_actual_tokens]
            if self.tree_node_depths_cpu is not None
            else None,
        )
