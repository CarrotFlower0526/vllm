...
class InputBatch:
    def __init__(
        ...
        # req_id -> diagnostic-only canonical token replay configuration.
        self.canonical_token_replays: dict[str, CanonicalTokenReplayConfig] = {}
    ...
    def add_request(
        ...
        canonical_token_replay = None
        if request.sampling_params is not None:
            canonical_token_replay = parse_canonical_token_replay(
                request.sampling_params
            )
            if canonical_token_replay is not None:
                canonical_token_replay.validate_vocab_size(self.vocab_size)
        ...
        if canonical_token_replay is not None:
            self.canonical_token_replays[req_id] = canonical_token_replay
    ...
    def remove_request(self, req_id: str) -> int | None:
        ...
        self.canonical_token_replays.pop(req_id, None)
    ...
    def _make_sampling_metadata(self) -> SamplingMetadata:
        ...
        needs_output_token_ids = (
            ...
            or bool(self.canonical_token_replays)
        )
        output_token_ids = (
        ...
        canonical_token_replay = None
        if self.canonical_token_replays:
            canonical_token_replay = CanonicalTokenReplayLogitsProcessor(
                [self.canonical_token_replays.get(req_id) for req_id in self.req_ids],
                cast(list[list[int]], self.req_output_token_ids),
            )
        ...
        return SamplingMetadata(
            ...
            canonical_token_replay=canonical_token_replay,
        )
