...
class Worker(WorkerBase):
    def __init__(
        ...
        if (
            ...
            and bool(getattr(speculative_config, "residual_tree", False))
        ):
            raise ValueError(
                "residual-tree speculative decoding is not implemented by "
                "the V2 model runner; set VLLM_USE_V2_MODEL_RUNNER=0 so "
                "no-spec, Stock-tree, and residual-tree latency arms can be "
                "measured on the same V1 execution system"
            )
    ...
    def set_residual_tree_runtime_config(
        ...
        """Switch residual-tree serving controls between completed batches."""

        drafter = getattr(self.model_runner, "drafter", None)
        setter = getattr(drafter, "set_residual_tree_runtime_config", None)
        if setter is None:
            raise ValueError(
                "the active model runner does not support residual-tree "
                "runtime controls"
            )
        normalized = setter(config)
        rejection_sampler = getattr(
            self.model_runner,
            "rejection_sampler",
            None,
        )
        if rejection_sampler is not None:
            rejection_sampler.residual_tree_trace_path = normalized.get(
                "trace_path"
            )
        return normalized
