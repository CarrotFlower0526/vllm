...
class GPUModelRunner(
    ...
    def __init__(
        ...
        self._draft_token_ids: (
            list[list[int]] | torch.Tensor | TreeDraftTokenIds | None
        ) = None
        self._draft_probs: torch.Tensor | None = None
        self._draft_prob_req_ids: list[str] | None = None
        self._draft_prob_num_rows: list[int] | None = None
        self._residual_tree_root_probability_trace_dir = os.environ.get(
            ROOT_VERIFIER_PROB_TRACE_ENV
        )
    ...
    def _update_states_after_model_execute(
        ...
        *,
        ...
        if tree_spec_decode and self.cache_config.mamba_cache_mode == "align":
            # Tree verification stores every branch in a separate recurrent
            # state slot. _compact_tree_spec_mamba_states has already copied
            # the final accepted branch to the canonical running-state slot.
            # The normal hybrid speculative postprocess assumes a *linear*
            # temporal layout and would overwrite that state using the wrong
            # branch. The align-mode preprocess consumes this CPU value as a
            # local state-slot offset, so tell it that the running state is
            # already canonical.
            self.input_batch.num_accepted_tokens_cpu_tensor[:num_reqs].fill_(1)
    ...
    def _compact_tree_spec_kv_cache(
        self,
        ...
        slot_mappings: dict[str, torch.Tensor] | list[dict[str, torch.Tensor]] | None,
        *,
        cache_slots_are_prevalidated: bool = False,
    ) -> tuple[torch.Tensor, torch.Tensor] | None:
        if not isinstance(spec_decode_metadata, TreeSpecDecodeMetadata):
            return None
        accepted_node_ids = sampler_output.accepted_tree_node_ids
        if accepted_node_ids is None:
            raise ValueError("tree sampler output is missing accepted node ids")
        if slot_mappings is None or isinstance(slot_mappings, list):
            raise NotImplementedError(
                "tree KV compaction requires unsplit per-layer slot mappings"
            )

        accepted_node_ids_cpu = accepted_node_ids.detach().cpu().tolist()
        src_flat_indices: list[int] = []
        dst_flat_indices: list[int] = []
        mamba_req_indices: list[int] = []
        mamba_src_local_indices: list[int] = []
        row_offset = 0
        for req_idx, req_id in enumerate(self.input_batch.req_ids):
            query_len = scheduler_output.num_scheduled_tokens[req_id]
            tree = scheduler_output.scheduled_spec_decode_token_trees.get(req_id)
            if tree is not None:
                final_accepted_node_id = 0
                for accepted_pos, node_id in enumerate(accepted_node_ids_cpu[req_idx]):
                    if node_id < 0:
                        break
                    if node_id >= query_len or accepted_pos + 1 >= query_len:
                        raise ValueError("accepted tree node id is out of range")
                    src = row_offset + int(node_id)
                    dst = row_offset + accepted_pos + 1
                    final_accepted_node_id = int(node_id)
                    if src != dst:
                        src_flat_indices.append(src)
                        dst_flat_indices.append(dst)
                if final_accepted_node_id != 0:
                    mamba_req_indices.append(req_idx)
                    mamba_src_local_indices.append(final_accepted_node_id)
            row_offset += query_len

        self._compact_tree_spec_mamba_states(
            mamba_req_indices,
            mamba_src_local_indices,
            [0] * len(mamba_req_indices),
            state_blocks_are_prevalidated=cache_slots_are_prevalidated,
        )
        if not src_flat_indices:
            return None

        src_flat = torch.tensor(src_flat_indices, dtype=torch.long, device=self.device)
        dst_flat = torch.tensor(dst_flat_indices, dtype=torch.long, device=self.device)
        # Hybrid cache groups can deliberately expose different logical
        # layers as views of the same backing tensor. Their block tables are
        # independent, so deduplicating by data_ptr alone skips required
        # copies. Only aliases inside the same group share a mapping.
        seen_kv_views: set[tuple[int, int]] = set()
        for group_idx, group in enumerate(self.kv_cache_config.kv_cache_groups):
            group_spec = group.kv_cache_spec
            if not isinstance(
                group_spec,
                AttentionSpec | UniformTypeKVCacheSpecs,
            ):
                continue
            for layer_name in group.layer_names:
                layer_spec = (
                    group_spec.kv_cache_specs.get(layer_name)
                ...
                if getattr(layer.impl, "_is_per_token_head_quant", False):
                    raise NotImplementedError(
                        "tree KV compaction does not support per-token-head KV "
                        "quantization yet because accepted-node copies must "
                        "also move the side scale caches"
                    )
                kv_cache = layer.kv_cache
                if not isinstance(kv_cache, torch.Tensor):
                    continue
                ptr = kv_cache.data_ptr()
                view_key = (group_idx, ptr)
                if view_key in seen_kv_views:
                    continue
                seen_kv_views.add(view_key)

                slot_mapping = slot_mappings[layer_name]
                src_slots = slot_mapping[src_flat]
                dst_slots = slot_mapping[dst_flat]
                self._copy_tree_kv_slots(
                    kv_cache,
                    src_slots,
                    dst_slots,
                    slots_are_prevalidated=cache_slots_are_prevalidated,
                )

        return src_flat, dst_flat

    def _compact_tree_spec_output_tensors(
        self,
        compaction_indices: tuple[torch.Tensor, torch.Tensor],
        hidden_states: torch.Tensor,
        sample_hidden_states: torch.Tensor,
        aux_hidden_states: list[torch.Tensor] | None,
    ) -> None:
        src_flat, dst_flat = compaction_indices
        self._copy_tree_tensor_rows(self.input_ids.gpu, src_flat, dst_flat)
        if self.enable_prompt_embeds:
            self._copy_tree_tensor_rows(self.inputs_embeds.gpu, src_flat, dst_flat)
            self._copy_tree_tensor_rows(self.is_token_ids.gpu, src_flat, dst_flat)

        if self.uses_mrope:
            self._copy_tree_tensor_cols(self.mrope_positions.gpu, src_flat, dst_flat)
        elif self.uses_xdrope_dim > 0:
            self._copy_tree_tensor_cols(self.xdrope_positions.gpu, src_flat, dst_flat)
        else:
            # Plain RoPE positions are a persistent CUDA tensor in the V1
            # runner.  Only M-RoPE/XD-RoPE use CPU/GPU buffer wrappers.
            self._copy_tree_tensor_rows(self.positions, src_flat, dst_flat)

        self._copy_tree_tensor_rows(hidden_states, src_flat, dst_flat)
        self._copy_tree_tensor_rows(sample_hidden_states, src_flat, dst_flat)
        if aux_hidden_states is not None:
            for aux_hidden_state in aux_hidden_states:
                self._copy_tree_tensor_rows(aux_hidden_state, src_flat, dst_flat)

    @staticmethod
    def _copy_tree_tensor_rows(
        tensor: torch.Tensor,
        src_rows: torch.Tensor,
        dst_rows: torch.Tensor,
    ) -> None:
        values = tensor.index_select(0, src_rows).clone()
        tensor.index_copy_(0, dst_rows, values)

    @staticmethod
    def _copy_tree_tensor_cols(
        tensor: torch.Tensor,
        src_cols: torch.Tensor,
        dst_cols: torch.Tensor,
    ) -> None:
        values = tensor.index_select(1, src_cols).clone()
        tensor.index_copy_(1, dst_cols, values)

    def _compact_tree_spec_mamba_states(
        self,
        req_indices_cpu: list[int],
        src_local_indices_cpu: list[int],
        dst_local_indices_cpu: list[int],
        *,
        state_blocks_are_prevalidated: bool = False,
    ) -> None:
        if not req_indices_cpu:
            return
        if not any(
            ...
            return
        if self.cache_config.mamba_cache_mode not in ("align", "none"):
            raise NotImplementedError(
                "tree Mamba state compaction currently supports none/align modes only"
            )

        num_reqs = self.input_batch.num_reqs
        req_indices = torch.tensor(
            req_indices_cpu, dtype=torch.long, device=self.device
        )
        src_local_indices = torch.tensor(
            src_local_indices_cpu, dtype=torch.long, device=self.device
        )
        dst_local_indices = torch.tensor(
            dst_local_indices_cpu, dtype=torch.long, device=self.device
        )
        max_local_index = max(
            max(src_local_indices_cpu),
            max(dst_local_indices_cpu),
        )

        # See the attention-cache note above: Mamba groups may share backing
        # storage while using disjoint block-table namespaces.
        seen_state_views: set[tuple[int, int]] = set()
        for group_idx, group in enumerate(self.kv_cache_config.kv_cache_groups):
            if not isinstance(group.kv_cache_spec, MambaSpec):
                continue
            block_table = self.input_batch.block_table[group_idx].get_device_tensor(
                num_reqs
            )
            ...
            if max_local_index >= state_block_table.shape[1]:
                raise NotImplementedError(
                    "tree Mamba state compaction needs one state block per "
                    "scheduled tree node"
                )

            src_blocks = state_block_table[
                req_indices,
                src_local_indices,
            ].long()
            dst_blocks = state_block_table[
                req_indices,
                dst_local_indices,
            ].long()
            if not state_blocks_are_prevalidated and (
                torch.any(src_blocks <= NULL_BLOCK_ID)
                or torch.any(dst_blocks <= NULL_BLOCK_ID)
            ):
                raise ValueError(
                    "tree Mamba state compaction received an invalid block id"
                )
            ...
            for layer_name in group.layer_names:
                ...
                for state in kv_caches:
                    ...
                    ptr = state.data_ptr()
                    view_key = (group_idx, ptr)
                    if view_key in seen_state_views:
                        continue
                    seen_state_views.add(view_key)
                    values = state.index_select(0, src_blocks).clone()
                    state.index_copy_(0, dst_blocks, values)

    @staticmethod
    def _copy_tree_kv_slots(
        kv_cache: torch.Tensor,
        src_slots: torch.Tensor,
        dst_slots: torch.Tensor,
        *,
        slots_are_prevalidated: bool = False,
    ) -> None:
        if kv_cache.ndim < 3 or kv_cache.shape[1] != 2:
            raise NotImplementedError(
                "tree KV compaction currently supports Triton-style "
                "[num_blocks, 2, block_size, ...] KV caches only"
            )
        block_size = kv_cache.shape[2]
        if not slots_are_prevalidated and (
            torch.any(src_slots < 0) or torch.any(dst_slots < 0)
        ):
            raise ValueError("tree KV compaction received an invalid slot")

        src_blocks = torch.div(src_slots, block_size, rounding_mode="floor").long()
        dst_blocks = torch.div(dst_slots, block_size, rounding_mode="floor").long()
        src_offsets = (src_slots % block_size).long()
        dst_offsets = (dst_slots % block_size).long()

        values = kv_cache[src_blocks, :, src_offsets].clone()
        kv_cache[dst_blocks, :, dst_offsets] = values
    ...
    def _prepare_inputs(
        ...
        torch.Tensor,
        SpecDecodeMetadata | TreeSpecDecodeMetadata | None,
    ]:
        """
        Returns:
        ...
        scheduled_token_trees = scheduler_output.scheduled_spec_decode_token_trees
        use_tree_spec_decode = len(scheduled_token_trees) > 0
        tree_attn_mask: torch.Tensor | None = None
        tree_position_offsets: np.ndarray | None = None
        tree_parent_local_indices_cpu: torch.Tensor | None = None
        tree_node_depths_cpu: torch.Tensor | None = None
        if use_tree_spec_decode:
            if get_pp_group().world_size > 1:
                raise NotImplementedError(
                    "SpecInfer tree verification does not support pipeline "
                    "parallelism yet"
                )
            if self.use_async_scheduling:
                raise NotImplementedError(
                    "SpecInfer tree verification does not support async "
                    "scheduling yet"
                )
            if self.use_async_spec_decode:
                raise NotImplementedError(
                    "SpecInfer tree verification does not support async "
                    "speculative scheduling yet"
                )
            if self.cache_config.kv_sharing_fast_prefill:
                raise NotImplementedError(
                    "SpecInfer tree verification does not support "
                    "KV-sharing fast prefill yet"
                )
            if self.model_config.enable_return_routed_experts:
                raise NotImplementedError(
                    "SpecInfer tree verification does not support routed "
                    "expert outputs yet"
                )
            if self.uses_mrope or self.uses_xdrope_dim > 0:
                for req_idx, req_id in enumerate(self.input_batch.req_ids):
                    if num_scheduled_tokens[req_idx] == 0:
                        continue
                    ...
                    if (
                        ...
                        raise NotImplementedError(
                            "SpecInfer tree verification with M-RoPE/XD-RoPE "
                            "currently supports decode-only batches"
                        )
            if (
                self.model_config.is_hybrid
                and self.cache_config.mamba_cache_mode not in ("align", "none")
            ):
                raise NotImplementedError(
                    "SpecInfer tree verification for hybrid state models "
                    "currently supports mamba_cache_mode='none' or 'align'"
                )
            (
                tree_attn_mask,
                tree_position_offsets,
                tree_parent_local_indices_cpu,
                tree_node_depths_cpu,
            ) = (
                self._calc_tree_attention_inputs(
                    scheduled_token_trees,
        ...
        if self.uses_mrope:
            ...
            if tree_position_offsets is not None:
                ...
                mrope_deltas = np.zeros(num_reqs, dtype=self.mrope_positions.np.dtype)
                for req_idx, req_id in enumerate(self.input_batch.req_ids):
                    req = self.requests[req_id]
                    assert req.mrope_position_delta is not None
                    mrope_deltas[req_idx] = req.mrope_position_delta
                tree_mrope_positions = tree_positions_np + mrope_deltas[req_indices]
                self.mrope_positions.np[:, : int(cu_num_tokens[-1])] = (
                    tree_mrope_positions
                )
        ...
        if self.uses_xdrope_dim > 0:
            ...
            if tree_position_offsets is not None:
                tree_xdrope_positions = (
                    self.input_batch.num_computed_tokens_cpu[req_indices]
                    + tree_position_offsets
                )
                self.xdrope_positions.np[:, : int(cu_num_tokens[-1])] = (
                    tree_xdrope_positions
                )
        ...
        if tree_position_offsets is not None:
            ...
            tree_positions = torch.from_numpy(tree_positions_np).to(
                self.device, dtype=torch.int64, non_blocking=True
            )
            self.positions[:total_num_scheduled_tokens].copy_(
                tree_positions, non_blocking=True
            )
        ...
        if not use_spec_decode:
            ...
            if use_tree_spec_decode:
                ...
                tree_input_rows = spec_decode_metadata.logits_indices.long()
                spec_decode_metadata.input_token_ids = self.input_ids.gpu.index_select(
                    0, tree_input_rows
                )
                prepared_positions = self._get_positions(
                    total_num_scheduled_tokens
                )
                if prepared_positions.ndim == 1:
                    tree_input_positions = prepared_positions.index_select(
                        0, tree_input_rows
                    )[:, None]
                else:
                    tree_input_positions = prepared_positions.index_select(
                ...
                spec_decode_metadata.input_positions = tree_input_positions

                attention_slot_columns: list[torch.Tensor] = []
                mamba_block_columns: list[torch.Tensor] = []
                attention_group_indices: list[int] = []
                mamba_group_indices: list[int] = []
                node_req_indices = (
                    spec_decode_metadata.node_request_indices.long()
                )
                global_node_indices = torch.arange(
                    spec_decode_metadata.node_token_ids.numel(),
                    dtype=torch.long,
                    device=self.device,
                )
                root_node_ids = spec_decode_metadata.root_node_ids.long()
                local_node_indices = global_node_indices - root_node_ids.index_select(
                    0, node_req_indices
                )
                for group_idx, group in enumerate(
                    ...
                    if isinstance(group.kv_cache_spec, AttentionSpec):
                        attention_group_indices.append(group_idx)
                        attention_slot_columns.append(
                            self.input_batch.block_table[
                                group_idx
                            ].slot_mapping.gpu.index_select(0, tree_input_rows)
                        )
                    elif isinstance(group.kv_cache_spec, MambaSpec):
                        mamba_group_indices.append(group_idx)
                        ...
                        mamba_block_columns.append(
                            state_block_table[
                                node_req_indices,
                                local_node_indices,
                            ]
                        )
                if attention_slot_columns:
                    spec_decode_metadata.input_attention_slots = torch.stack(
                        attention_slot_columns, dim=1
                    )
                    spec_decode_metadata.input_attention_group_indices = (
                        attention_group_indices
                    )
                if mamba_block_columns:
                    spec_decode_metadata.input_mamba_state_blocks = torch.stack(
                        mamba_block_columns, dim=1
                    )
                    spec_decode_metadata.input_mamba_group_indices = (
                        mamba_group_indices
                    )
                spec_decode_metadata.tree_attn_mask = tree_attn_mask
                spec_decode_metadata.tree_parent_local_indices_cpu = (
                    tree_parent_local_indices_cpu
                )
                spec_decode_metadata.tree_node_depths_cpu = tree_node_depths_cpu
            else:
    ...
    def _build_attention_metadata(
        ...
        cm_base = CommonAttentionMetadata(
            ...
            tree_parent_local_indices_cpu=tree_parent_local_indices_cpu,
            tree_node_depths_cpu=tree_node_depths_cpu,
        )
        ...
        def _build_attn_group_metadata(
            ...
            if for_cudagraph_capture:
                ...
                if (
                    common_attn_metadata.tree_attn_mask is not None
                    and type(builder) is not TritonAttentionMetadataBuilder
                    and not isinstance(builder, GDNAttentionMetadataBuilder)
                ):
                    raise NotImplementedError(
                        "SpecInfer tree attention currently requires the "
                        "Triton attention backend, or GDN state metadata for "
                        "hybrid state layers"
                    )
    ...
    def _calc_tree_attention_inputs(
        self,
        scheduled_token_trees: dict[str, DraftTokenTree],
        num_scheduled_tokens: np.ndarray,
        total_num_scheduled_tokens: int,
    ) -> tuple[torch.Tensor, np.ndarray, torch.Tensor, torch.Tensor]:
        max_query_len = max(1, int(num_scheduled_tokens.max()))
        tree_attn_mask = np.zeros(
            (total_num_scheduled_tokens, max_query_len), dtype=np.bool_
        )
        tree_position_offsets = np.empty(
            total_num_scheduled_tokens, dtype=np.int64
        )
        tree_parent_local_indices = np.empty(
            total_num_scheduled_tokens, dtype=np.int32
        )
        tree_node_depths = np.empty(total_num_scheduled_tokens, dtype=np.int32)

        row_offset = 0
        for req_idx, req_id in enumerate(self.input_batch.req_ids):
            query_len = int(num_scheduled_tokens[req_idx])
            tree = scheduled_token_trees.get(req_id)
            if tree is None:
                for local_pos in range(query_len):
                    row = row_offset + local_pos
                    tree_attn_mask[row, : local_pos + 1] = True
                    tree_position_offsets[row] = local_pos
                    tree_parent_local_indices[row] = local_pos - 1
                    tree_node_depths[row] = local_pos
                row_offset += query_len
                continue

            if tree.num_tree_nodes != query_len:
                raise ValueError("scheduled tree node count does not match tokens")

            tree_position_offsets[
                row_offset : row_offset + query_len
            ] = np.asarray(tree.node_depths, dtype=np.int64)
            tree_parent_local_indices[
                row_offset : row_offset + query_len
            ] = np.asarray(tree.parent_node_ids, dtype=np.int32)
            tree_node_depths[
                row_offset : row_offset + query_len
            ] = np.asarray(tree.node_depths, dtype=np.int32)
            for local_node_id in range(tree.num_tree_nodes):
                row = row_offset + local_node_id
                cursor = local_node_id
                while cursor >= 0:
                    tree_attn_mask[row, cursor] = True
                    cursor = tree.parent_node_ids[cursor]
            row_offset += query_len

        if row_offset != total_num_scheduled_tokens:
            raise ValueError("tree attention input size does not match batch")

        return (
            async_tensor_h2d(tree_attn_mask, device=self.device),
            tree_position_offsets,
            torch.from_numpy(tree_parent_local_indices),
            torch.from_numpy(tree_node_depths),
        )
    ...
    def _calc_tree_spec_decode_metadata(
        ...
        num_sampled_tokens = num_draft_tokens + 1
        cu_num_tree_nodes = self._get_cumsum_and_arange(
            num_sampled_tokens, self._arange_scratch, cumsum_dtype=np.int32
        )
        ...
        logits_indices_np += self._arange_scratch[: cu_num_tree_nodes[-1]]
        ...
        proposal_offset = 0
        for req_idx, req_id in enumerate(self.input_batch.req_ids):
            expected_draft_tokens = int(num_draft_tokens[req_idx])
            tree = scheduled_token_trees.get(req_id)
            if tree is None:
                if expected_draft_tokens != 0:
                    raise ValueError(
                        "linear and tree speculative metadata cannot be mixed"
                    )
                tree = DraftTokenTree.root_only()
            if tree.num_draft_tokens != expected_draft_tokens:
                raise ValueError("scheduled tree draft count does not match tokens")
            ...
            for local_node_id in range(tree.num_tree_nodes):
                ...
                parent_node_ids.append(
                    -1 if parent_id < 0 else node_offset + parent_id
                )
                ...
                child_node_ids.extend(
                    node_offset + child_id
                    for child_id in tree.child_node_ids[child_start:child_end]
                )
            ...
            for child_id, row, head_id, score in zip(
                ...
                contributor_proposal_rows.append(
                    -1 if row < 0 else proposal_offset + row
                )
        ...
        device = self.device
        cu_num_tree_nodes_tensor = async_tensor_h2d(cu_num_tree_nodes, device=device)
        logits_indices = async_tensor_h2d(logits_indices_np, device=device)
        target_logits_indices = torch.arange(
            len(node_token_ids), dtype=torch.int32, device=device
        )

        return TreeSpecDecodeMetadata(
            node_token_ids=async_tensor_h2d(
                np.asarray(node_token_ids, dtype=np.int32), device=device
            ),
            parent_node_ids=async_tensor_h2d(
                np.asarray(parent_node_ids, dtype=np.int32), device=device
            ),
            node_request_indices=async_tensor_h2d(
                np.asarray(node_request_indices, dtype=np.int32), device=device
            ),
            node_depths=async_tensor_h2d(
                np.asarray(node_depths, dtype=np.int32), device=device
            ),
            node_priorities=async_tensor_h2d(
                np.asarray(node_priorities, dtype=np.float32), device=device
            ),
            root_node_ids=async_tensor_h2d(
                np.asarray(root_node_ids, dtype=np.int32), device=device
            ),
            num_draft_tokens=num_draft_tokens.tolist(),
            num_proposal_rows=num_proposal_rows,
            cu_num_tree_nodes=cu_num_tree_nodes_tensor,
            child_start_indices=async_tensor_h2d(
                np.asarray(child_start_indices, dtype=np.int32), device=device
            ),
            child_end_indices=async_tensor_h2d(
                np.asarray(child_end_indices, dtype=np.int32), device=device
            ),
            child_node_ids=async_tensor_h2d(
                np.asarray(child_node_ids, dtype=np.int32), device=device
            ),
            contributor_child_node_ids=async_tensor_h2d(
                np.asarray(contributor_child_node_ids, dtype=np.int32),
                device=device,
            ),
            contributor_proposal_rows=async_tensor_h2d(
                np.asarray(contributor_proposal_rows, dtype=np.int32),
                device=device,
            ),
            contributor_head_ids=async_tensor_h2d(
                np.asarray(contributor_head_ids, dtype=np.int32), device=device
            ),
            contributor_scores=async_tensor_h2d(
                np.asarray(contributor_scores, dtype=np.float32), device=device
            ),
            target_logits_indices=target_logits_indices,
            logits_indices=logits_indices,
            trace_ids=trace_ids,
    ...
    def _sample(
        ...
        if canonical_replay is not None:
            # Defense in depth for direct sampling entry points and unit tests;
            # execute_model performs the same check before target forward.
            canonical_replay.validate_plain_target_step(spec_decode_metadata)
    ...
    def execute_model(
        ...
        if (
            ...
            spec_decode_token_trees_copy = (
                scheduler_output.scheduled_spec_decode_token_trees.copy()
            )
            scheduler_output = replace(
                scheduler_output,
                num_scheduled_tokens=num_scheduled_tokens_copy,
                scheduled_spec_decode_tokens=spec_decode_tokens_copy,
                scheduled_spec_decode_token_trees=spec_decode_token_trees_copy,
            )
        ...
        with (
            ...
            canonical_replay = (
                self.input_batch.sampling_metadata.canonical_token_replay
            )
            if canonical_replay is not None:
                # Fail before the target-model forward, rather than relying on
                # a later trace analyzer to discover accidental verifier work.
                canonical_replay.validate_plain_target_step(spec_decode_metadata)
            use_tree_spec_decode = isinstance(
                spec_decode_metadata, TreeSpecDecodeMetadata
            )
            ...
            (
                ...
                allow_microbatching=not use_tree_spec_decode,
                force_eager=use_tree_spec_decode,
            ...
            use_spec_decode = len(scheduler_output.scheduled_spec_decode_tokens) > 0
            tree_attn_mask = (
                spec_decode_metadata.tree_attn_mask
            ...
            tree_node_depths_cpu = (
                spec_decode_metadata.tree_node_depths_cpu
                if isinstance(spec_decode_metadata, TreeSpecDecodeMetadata)
                else None
            )
        ...
        self._maybe_trace_target_root_numerics(
            logits=logits,
            logits_indices=logits_indices,
            spec_decode_metadata=spec_decode_metadata,
            sample_hidden_states=sample_hidden_states,
        )
        if self._residual_tree_root_probability_trace_dir:
            self._maybe_capture_target_root_probabilities(
                logits=logits,
                spec_decode_metadata=spec_decode_metadata,
            )
    ...
    def _maybe_trace_target_root_numerics(
        self,
        *,
        logits: torch.Tensor,
        logits_indices: torch.Tensor,
        ...
        """Write an expensive, opt-in exact numeric trace for root rows."""

        trace_path = os.environ.get(NUMERIC_TRACE_ENV)
        if not trace_path:
            return
        ...
        if isinstance(spec_decode_metadata, TreeSpecDecodeMetadata):
            mode = "tree"
            root_rows = spec_decode_metadata.root_node_ids.long()
            sample_input_token_ids = spec_decode_metadata.input_token_ids
            sample_positions = spec_decode_metadata.input_positions
        elif isinstance(spec_decode_metadata, SpecDecodeMetadata):
            mode = "linear_spec"
            cumulative = spec_decode_metadata.cu_num_sampled_tokens.long()
            root_rows = torch.cat(
                (
                    cumulative.new_zeros(1),
                    cumulative[:-1],
                )
            )
            sample_input_rows = logits_indices.long()
            sample_input_token_ids = self.input_ids.gpu.index_select(
                0, sample_input_rows
            )
            sample_positions = self.positions.index_select(
                0, sample_input_rows
            ).unsqueeze(1)
        else:
            mode = "no_spec"
            root_rows = torch.arange(
                num_reqs,
                dtype=torch.long,
        ...
        if root_rows.numel() != num_reqs:
            raise ValueError(
                "numeric trace root rows do not match the active requests"
            )
        trace_step = getattr(self, "_residual_tree_numeric_trace_step", 0)
        append_target_numeric_trace(
            trace_path,
            step=trace_step,
            mode=mode,
            ...
            sample_hidden_states=sample_hidden_states,
            logits=logits,
            sample_input_token_ids=sample_input_token_ids,
            sample_positions=sample_positions,
        )
        self._residual_tree_numeric_trace_step = trace_step + 1

    def _residual_tree_draft_target_ids(
        self,
        *,
        ...
        cached = getattr(
            self,
            "_residual_tree_root_probability_support_ids",
            None,
        )
        ...
        draft_vocab_size = int(
            getattr(draft_config, "draft_vocab_size", target_vocab_size)
        )
        mapping = getattr(draft_model, "draft_id_to_target_id", None)
        if mapping is None:
            if draft_vocab_size != target_vocab_size:
                raise ValueError(
                    "root verifier probability capture requires the drafter's "
            ...
            if int(offsets.numel()) != draft_vocab_size:
                raise ValueError(
                    "draft_id_to_target_id does not match draft_vocab_size"
                )
            target_ids = (
                torch.arange(
                    draft_vocab_size,
        ...
        if bool(((target_ids < 0) | (target_ids >= target_vocab_size)).any().item()):
            raise ValueError("draft_id_to_target_id maps outside the target vocabulary")
        if int(torch.unique(target_ids).numel()) != int(target_ids.numel()):
            raise ValueError("draft_id_to_target_id contains duplicate target ids")
        self._residual_tree_root_probability_support_ids = target_ids
        return target_ids

    def _maybe_capture_target_root_probabilities(
        self,
        *,
        logits: torch.Tensor,
        spec_decode_metadata: SpecDecodeMetadata | TreeSpecDecodeMetadata | None,
    ) -> None:
        """Persist an opt-in root target-distribution sidecar."""

        destination = self._residual_tree_root_probability_trace_dir
        if not destination:
            return
        if not isinstance(spec_decode_metadata, TreeSpecDecodeMetadata):
            return

        trace_ids = spec_decode_metadata.trace_ids
        if trace_ids is None or any(trace_id is None for trace_id in trace_ids):
            raise ValueError(
                "root verifier probability capture requires residual-tree trace ids"
            )
        request_ids = [str(value) for value in self.input_batch.req_ids]
        if len(trace_ids) != len(request_ids):
            raise ValueError(
                "root verifier probability trace ids do not match requests"
            )
        draft_target_ids = self._residual_tree_draft_target_ids(
            target_vocab_size=int(logits.shape[-1]),
            device=logits.device,
        )
        capture_step = getattr(
            self,
            "_residual_tree_root_probability_capture_step",
            0,
        )
        append_residual_tree_root_verifier_probabilities(
            destination,
            ...
            root_row_indices=spec_decode_metadata.root_node_ids,
            logits=logits,
            draft_target_ids=draft_target_ids,
        )
        self._residual_tree_root_probability_capture_step = capture_step + 1

    def _maybe_trace_selected_tree_source_numerics(
        self,
        *,
        logits: torch.Tensor,
        spec_decode_metadata: SpecDecodeMetadata | TreeSpecDecodeMetadata | None,
        ...
        """Trace exact target rows that emitted the accepted tree path."""

        trace_path = os.environ.get(NUMERIC_TRACE_ENV)
        if not trace_path or not isinstance(
            spec_decode_metadata, TreeSpecDecodeMetadata
        ):
            return
        accepted_node_ids = sampler_output.accepted_tree_node_ids
        if accepted_node_ids is None:
            raise ValueError("numeric tree trace is missing accepted node ids")

        accepted_cpu = accepted_node_ids.detach().cpu().tolist()
        sampled_cpu = sampler_output.sampled_token_ids.detach().cpu().tolist()
        root_ids = spec_decode_metadata.root_node_ids.detach().cpu().tolist()
        depths = spec_decode_metadata.node_depths.detach().cpu().tolist()
        parent_ids = spec_decode_metadata.parent_node_ids.detach().cpu().tolist()
        request_ids: list[str] = []
        source_rows: list[int] = []
        record_metadata: list[dict[str, Any]] = []

        for req_idx, request_id in enumerate(self.input_batch.req_ids):
            valid_outputs = [
                int(token_id)
                for token_id in sampled_cpu[req_idx]
                if int(token_id) >= 0
            ]
            accepted_local = [
                int(node_id)
                for node_id in accepted_cpu[req_idx]
                if int(node_id) >= 0
            ]
            source_local_ids = [0, *accepted_local]
            if len(source_local_ids) != len(valid_outputs):
                raise ValueError(
                    "numeric trace source nodes do not match sampled outputs"
                )
            root_id = int(root_ids[req_idx])
            for output_index, (local_node_id, sampled_token_id) in enumerate(
                zip(source_local_ids, valid_outputs, strict=True)
            ):
                source_row = root_id + local_node_id
                request_ids.append(request_id)
                source_rows.append(source_row)
                global_parent_id = int(parent_ids[source_row])
                record_metadata.append(
                    {
                        "request_index": req_idx,
                        "output_index_in_step": output_index,
                        "source_local_node_id": local_node_id,
                        "source_parent_local_node_id": (
                            -1
                            if global_parent_id < 0
                            else global_parent_id - root_id
                        ),
                        "source_depth": int(depths[source_row]),
        ...
        trace_step = max(
            0,
            getattr(self, "_residual_tree_numeric_trace_step", 1) - 1,
        )
        append_target_numeric_trace(
            trace_path,
            step=trace_step,
            mode="tree_selected_sources",
            event="target_selected_source_numeric",
            request_ids=request_ids,
    ...
    def sample_tokens(
        ...
        self._maybe_trace_selected_tree_source_numerics(
            logits=logits,
            spec_decode_metadata=spec_decode_metadata,
            sample_hidden_states=sample_hidden_states,
        ...
        tree_compaction_indices = self._compact_tree_spec_kv_cache(
            sampler_output,
            scheduler_output,
            spec_decode_metadata,
            slot_mappings,
            # These mappings come from the scheduler-owned block tables for
            # the exact unpadded rows validated in _prepare_inputs. The host
            # path walk above bounds-checks every selected row, so repeating a
            # CUDA-to-host negative-slot check in every KV layer is redundant.
            cache_slots_are_prevalidated=True,
        )
        if tree_compaction_indices is not None:
            self._compact_tree_spec_output_tensors(
                tree_compaction_indices,
                hidden_states,
                sample_hidden_states,
                aux_hidden_states,
            )
        ...
        self._draft_prob_num_rows = None
        ...
        def propose_draft_token_ids(sampled_token_ids):
            ...
            replay = self.input_batch.sampling_metadata.canonical_token_replay
            proposal_only = (
                replay is not None and replay.proposal_only_batch()
            )
            with record_function_or_nullcontext("gpu_model_runner: draft"):
                if proposal_only and replay.proposal_only_final_step():
                    # The token just sampled is the final reference token.
                    # Do not run a drafter whose proposal can never be
                    # consumed or measured.
                    if spec_config is not None and spec_config.residual_tree:
                        self._draft_token_ids = TreeDraftTokenIds(
                            req_ids=self.input_batch.req_ids.copy(),
                            draft_token_trees=[
                                DraftTokenTree.root_only()
                    ...
                    if proposal_only:
                        # Capture the proposal against the *next* canonical
                        # token, then erase it before the scheduler can turn it
                        # into a verifier query.  Consequently every target
                        # step in this diagnostic remains an ordinary one-token
                        # target-model step.
                        if isinstance(self._draft_token_ids, TreeDraftTokenIds):
                            replay.trace_proposal_only_trees(
                                self._draft_token_ids.draft_token_trees,
                                self._draft_token_ids.req_ids,
                            )
                            self._draft_token_ids = TreeDraftTokenIds(
                                req_ids=self._draft_token_ids.req_ids,
                                draft_token_trees=[
                                    DraftTokenTree.root_only()
                                    for _ in self._draft_token_ids.req_ids
                                ],
                            )
                        else:
                            if torch.is_tensor(self._draft_token_ids):
                                ...
                                proposal_rows = self._draft_token_ids
                            else:
                                raise TypeError(
                                    "proposal-only replay received an "
                                    "unsupported draft-token container"
                                )
                            replay.trace_proposal_only_linear(
                                proposal_rows,
        ...
        if spec_config is not None:
            ...
            replay = self.input_batch.sampling_metadata.canonical_token_replay
            if replay is not None and replay.proposal_only_batch() and not (
                use_gpu_toks and spec_config.use_eagle()
            ):
                raise NotImplementedError(
                    "canonical proposal-only replay currently requires the "
                    "padded GPU EAGLE proposer so proposal offsets are captured "
                    "before output bookkeeping"
                )
            if use_gpu_toks:
    ...
    def take_draft_token_ids(self) -> DraftTokenIds | TreeDraftTokenIds | None:
        if isinstance(self._draft_token_ids, TreeDraftTokenIds):
    ...
    def _get_spec_decode_draft_probs(
        self, spec_decode_metadata: SpecDecodeMetadata | TreeSpecDecodeMetadata
    ) -> torch.Tensor | None:
        if isinstance(spec_decode_metadata, TreeSpecDecodeMetadata):
            if self._draft_probs is None:
                return None
            if self._draft_prob_req_ids is None or self._draft_prob_num_rows is None:
                raise ValueError(
                    "tree draft probabilities are missing request row metadata"
                )
            row_ranges: dict[str, tuple[int, int]] = {}
            cursor = 0
            for req_id, num_rows in zip(
                self._draft_prob_req_ids, self._draft_prob_num_rows
            ):
                row_ranges[req_id] = (cursor, cursor + num_rows)
                cursor += num_rows
            if cursor != self._draft_probs.shape[0]:
                raise ValueError("tree draft probability row count is inconsistent")

            reordered_rows: list[torch.Tensor] = []
            for req_id, scheduled_rows in zip(
                self.input_batch.req_ids, spec_decode_metadata.num_proposal_rows
            ):
                if scheduled_rows == 0:
                    continue
                row_range = row_ranges.get(req_id)
                if row_range is None:
                    raise ValueError(
                        "missing cached tree draft probability rows for "
                        f"request {req_id}"
                    )
                start, end = row_range
                scheduled_end = start + scheduled_rows
                if scheduled_end > end:
                    raise ValueError(
                        "scheduled tree references more proposal rows than "
                        "the cached draft tree contains"
                    )
                reordered_rows.append(self._draft_probs[start:scheduled_end])

            if not reordered_rows:
                return self._draft_probs.new_empty(
                    (0, self._draft_probs.shape[-1])
                )
            draft_probs = torch.cat(reordered_rows, dim=0).contiguous()
            proposal_rows = spec_decode_metadata.contributor_proposal_rows
            valid_rows = proposal_rows[proposal_rows >= 0]
            if valid_rows.numel() and int(valid_rows.max().item()) >= (
                draft_probs.shape[0]
            ):
                raise ValueError(
                    "tree metadata references a missing draft probability row"
                )
            return draft_probs
    ...
    def propose_draft_token_ids(
        ...
        common_attn_metadata: CommonAttentionMetadata,
        slot_mappings: dict[str, torch.Tensor] | list[dict[str, torch.Tensor]] | None,
    ) -> list[list[int]] | torch.Tensor | TreeDraftTokenIds:
        num_scheduled_tokens = scheduler_output.total_num_scheduled_tokens
        spec_config = self.speculative_config
        ...
        if spec_config.method == "ngram":
            ...
            if spec_config.residual_tree:
                if not isinstance(self.drafter, EagleProposer):
                    raise NotImplementedError(
                        "residual_tree drafting currently requires the "
                        "EAGLE proposer"
                    )
                draft_token_trees = self.drafter.propose_residual_tree(
                    num_speculative_tokens=num_spec_tokens_to_schedule,
                ...
                draft_token_ids = TreeDraftTokenIds(
                    req_ids=self.input_batch.req_ids.copy(),
                    draft_token_trees=draft_token_trees,
                )
            else:
                draft_token_ids = self.drafter.propose(
                    num_speculative_tokens=num_spec_tokens_to_schedule,
            ...
            if hasattr(self.drafter, "take_last_draft_probs"):
                ...
                if draft_probs is not None:
                    ...
                    if isinstance(draft_token_ids, TreeDraftTokenIds):
                        self._draft_prob_num_rows = [
                            tree.num_proposal_rows
                            for tree in draft_token_ids.draft_token_trees
                        ]
                    else:
                        self._draft_prob_num_rows = None
