...
class Scheduler(SchedulerInterface):
    def __init__(
        ...
        self.tree_verification_requires_decode_only = (
            vllm_config.model_config.uses_mrope
            or vllm_config.model_config.uses_xdrope_dim > 0
        )
    ...
    def schedule(self, throttle_prefills: bool = False) -> SchedulerOutput:
        ...
        scheduled_spec_decode_token_trees = {}
        ...
        while req_index < len(self.running) and token_budget > 0:
                ...
                while True:
                    ...
                    if self.policy == SchedulingPolicy.PRIORITY:
                        ...
                        if preempted_req in scheduled_running_reqs:
                            ...
                            scheduled_spec_decode_token_trees.pop(
            ...
            if request.spec_token_ids:
                ...
                if num_scheduled_spec_tokens > 0:
                    spec_token_tree = request.spec_token_tree
                    if spec_token_tree is not None:
                        spec_token_tree = spec_token_tree.truncate(
                            num_scheduled_spec_tokens
                        )
                        spec_token_ids = spec_token_tree.draft_token_ids
                        scheduled_spec_decode_token_trees[request.request_id] = (
                            spec_token_tree
                        )
                    else:
        ...
        if not preempted_reqs and self._pause_state == PauseState.UNPAUSED:
            ...
            # Tree verification currently requires a decode-only batch for
            # models using M-RoPE/XD-RoPE. Defer new prefills until the active
            # tree-verification batch drains instead of mixing the two modes.
            while (
                (self.waiting or self.skipped_waiting)
                and token_budget > 0
                and not (
                    scheduled_spec_decode_token_trees
                    and self.tree_verification_requires_decode_only
                )
            ):
        ...
        scheduler_output = SchedulerOutput(
            ...
            scheduled_spec_decode_token_trees=scheduled_spec_decode_token_trees,
    ...
    def update_draft_token_ids(
        self, draft_token_ids: DraftTokenIds | TreeDraftTokenIds
    ) -> None:
        if isinstance(draft_token_ids, TreeDraftTokenIds):
            self._update_tree_draft_token_ids(draft_token_ids)
            return
        ...
        for req_id, spec_token_ids in zip(
            ...
            if self.structured_output_manager.should_advance(request):
                ...
                grammar = metadata.grammar  # type: ignore[union-attr]
                spec_token_ids = grammar.validate_tokens(spec_token_ids)
            request.spec_token_ids = spec_token_ids
            request.spec_token_tree = None

    def _update_tree_draft_token_ids(self, draft_token_ids: TreeDraftTokenIds) -> None:
        for req_id, spec_token_tree in zip(
            draft_token_ids.req_ids,
            draft_token_ids.draft_token_trees,
        ):
            request = self.requests.get(req_id)
            if request is None or request.is_finished():
                continue
            ...
            if self.structured_output_manager.should_advance(request):
                ...
                grammar = metadata.grammar  # type: ignore[union-attr]
                spec_token_ids = grammar.validate_tokens(spec_token_ids)
                spec_token_tree = spec_token_tree.truncate(len(spec_token_ids))
            request.spec_token_ids = spec_token_ids
            request.spec_token_tree = spec_token_tree

    def update_draft_token_ids_in_output(
        self,
        draft_token_ids: DraftTokenIds | TreeDraftTokenIds,
        scheduler_output: SchedulerOutput,
    ) -> None:
        if isinstance(draft_token_ids, TreeDraftTokenIds):
            self._update_tree_draft_token_ids_in_output(
    ...
    def _update_tree_draft_token_ids_in_output(
        self,
            ...
            for req_id in draft_token_ids.req_ids:
                ...
                if should_advance:
                    raise NotImplementedError(
                        "SpecInfer tree drafts do not support deferred "
                        "structured-output validation yet"
                    )

        num_invalid_spec_tokens: dict[str, int] = {}

        sched_spec_tokens = scheduler_output.scheduled_spec_decode_tokens
        sched_spec_trees = scheduler_output.scheduled_spec_decode_token_trees
        for req_id, spec_token_tree in zip(
            draft_token_ids.req_ids,
            draft_token_ids.draft_token_trees,
        ):
            ...
            orig_num_spec_tokens = len(placeholder_spec_tokens)
            spec_token_tree = spec_token_tree.truncate(orig_num_spec_tokens)
            spec_token_ids = spec_token_tree.draft_token_ids
            if self.structured_output_manager.should_advance(request):
                metadata = request.structured_output_request
                assert metadata is not None and metadata.grammar is not None
            ...
            sched_spec_tokens[req_id] = spec_token_ids
            sched_spec_trees[req_id] = spec_token_tree
