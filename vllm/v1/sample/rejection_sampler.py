...
from vllm.v1.spec_decode.residual_tree import (
    ...
    verify_stochastic_tree_batch_with_nodes,
)
from vllm.v1.spec_decode.residual_tree_trace import append_residual_tree_trace
...
class RejectionSampler(nn.Module):
    ...
    def __init__(
        ...
        self.residual_tree_trace_path = (
            getattr(spec_config, "residual_tree_trace_path", None)
    ...
    def forward(
        self,
        metadata: SpecDecodeMetadata | TreeSpecDecodeMetadata,
        ...
        Returns:
            SamplerOutput:
                ...
                requested.
        """
        if isinstance(metadata, TreeSpecDecodeMetadata):
            return self._forward_tree(
                metadata,
                draft_probs,
                logits,
                sampling_metadata,
        ...
        canonical_token_replay = sampling_metadata.canonical_token_replay
        if canonical_token_replay is not None:
            target_logits = canonical_token_replay.apply_linear(
                target_logits,
        ...
        if canonical_token_replay is not None:
            canonical_token_replay.trace_linear(metadata, output_token_ids)
            output_token_ids = canonical_token_replay.truncate_linear_output(
                output_token_ids,
    ...
    def _forward_tree(
        ...
        proposal_probs: torch.Tensor | None,
        logits: torch.Tensor,
        sampling_metadata: SamplingMetadata,
    ) -> SamplerOutput:
        """SpecInfer-style tree verification path.

        Tree verification needs target logits for every tree node, not the
        linear target/bonus split used by standard speculative decoding.
        """

        if metadata.max_spec_len > MAX_SPEC_LEN:
            raise ValueError(f"tree draft length exceeds {MAX_SPEC_LEN}")
        if sampling_metadata.max_num_logprobs is not None:
            raise NotImplementedError("tree speculative logprobs are not wired yet")
        if not sampling_metadata.no_penalties:
            raise NotImplementedError("tree speculative penalties are not wired yet")
        if sampling_metadata.bad_words_token_ids:
            raise NotImplementedError(
                "tree speculative bad-words filtering is not wired yet"
            )
        if sampling_metadata.allowed_token_ids_mask is not None:
            raise NotImplementedError(
                "tree speculative allowed-token masks are not wired yet"
            )
        holder = sampling_metadata.thinking_budget_state_holder
        if holder is not None and holder.has_tracked_requests():
            raise NotImplementedError(
                "tree speculative thinking-budget logits are not wired yet"
            )
        unsupported_logit_processors = [
            processor
            for processor in sampling_metadata.logitsprocs.non_argmax_invariant
            if not isinstance(processor, MinTokensLogitsProcessor)
        ]
        if unsupported_logit_processors:
            raise NotImplementedError(
                "tree speculative logits processors are not wired yet"
            )
        if not (sampling_metadata.all_greedy or sampling_metadata.all_random):
            raise NotImplementedError(
                "mixed greedy/random tree verification is not wired yet"
            )

        target_logits = logits[metadata.target_logits_indices].to(torch.float32)
        if not self.is_processed_logprobs_mode:
            target_logits = target_logits.clone()
        target_logits = self.apply_tree_logits_processors(
            target_logits,
            sampling_metadata,
            metadata,
        )
        target_logits = self.apply_tree_sampling_constraints(
            target_logits,
            sampling_metadata,
            metadata,
        )
        canonical_token_replay = sampling_metadata.canonical_token_replay
        if canonical_token_replay is not None:
            target_logits = canonical_token_replay.apply_tree(
                target_logits,
                metadata.node_request_indices,
                metadata.node_depths,
            )

        trace_top2_logits: torch.Tensor | None = None
        trace_top2_token_ids: torch.Tensor | None = None
        target_token_ids = target_logits.argmax(dim=-1)
        if self.residual_tree_trace_path is not None:
            trace_top2_logits, trace_top2_token_ids = torch.topk(
                target_logits,
                k=2,
                dim=-1,
            )
        ...
        if sampling_metadata.all_greedy:
            sampled_token_ids, accepted_tree_node_ids = (
                verify_greedy_tree_batch_with_nodes(
                    metadata,
                    target_token_ids,
                    placeholder_token_id=PLACEHOLDER_TOKEN_ID,
            ...
            if proposal_probs is None:
                raise ValueError(
                    "tree stochastic verification requires proposal probabilities"
                )
        ...
        if self.residual_tree_trace_path is not None:
            self._write_residual_tree_verification_trace(
                metadata,
                target_token_ids,
                sampled_token_ids,
                accepted_tree_node_ids,
                greedy=bool(sampling_metadata.all_greedy),
                target_top2_token_ids=trace_top2_token_ids,
                target_top2_logits=trace_top2_logits,
            )
        if canonical_token_replay is not None:
            canonical_token_replay.trace_tree(
                metadata,
                sampled_token_ids,
                accepted_tree_node_ids,
            )
            sampled_token_ids, accepted_tree_node_ids = (
                canonical_token_replay.truncate_tree_output(
                    sampled_token_ids,
                    accepted_tree_node_ids,
                    placeholder_token_id=PLACEHOLDER_TOKEN_ID,
                )
        ...
        return SamplerOutput(
            ...
            logprobs_tensors=None,
            accepted_tree_node_ids=accepted_tree_node_ids,
        )

    def _write_residual_tree_verification_trace(
        self,
        metadata: TreeSpecDecodeMetadata,
        ...
        *,
        greedy: bool,
        target_top2_token_ids: torch.Tensor | None = None,
        target_top2_logits: torch.Tensor | None = None,
    ) -> None:
        trace_ids = metadata.trace_ids or [None] * len(metadata.num_draft_tokens)
        cu_nodes = metadata.cu_num_tree_nodes.detach().cpu().tolist()
        node_tokens = metadata.node_token_ids.detach().cpu().tolist()
        parent_ids = metadata.parent_node_ids.detach().cpu().tolist()
        depths = metadata.node_depths.detach().cpu().tolist()
        priorities = metadata.node_priorities.detach().cpu().tolist()
        input_tokens = (
            metadata.input_token_ids.detach().cpu().tolist()
            if metadata.input_token_ids is not None
            else [None] * len(node_tokens)
        )
        input_positions = (
            metadata.input_positions.detach().cpu().tolist()
            if metadata.input_positions is not None
            else [None] * len(node_tokens)
        )
        input_attention_slots = (
            metadata.input_attention_slots.detach().cpu().tolist()
            if metadata.input_attention_slots is not None
            else [None] * len(node_tokens)
        )
        input_mamba_state_blocks = (
            metadata.input_mamba_state_blocks.detach().cpu().tolist()
            if metadata.input_mamba_state_blocks is not None
            else [None] * len(node_tokens)
        )
        targets = target_token_ids.detach().cpu().tolist()
        top2_token_ids = (
            target_top2_token_ids.detach().cpu().tolist()
            if target_top2_token_ids is not None
            else None
        )
        top2_logits = (
            target_top2_logits.detach().cpu().tolist()
            if target_top2_logits is not None
            else None
        )
        sampled_rows = sampled_token_ids.detach().cpu().tolist()
        accepted_rows = accepted_tree_node_ids.detach().cpu().tolist()
        for request_index, trace_id in enumerate(trace_ids):
            if trace_id is None:
                continue
            start = 0 if request_index == 0 else int(cu_nodes[request_index - 1])
            end = int(cu_nodes[request_index])
            accepted = [
                int(node_id)
                for node_id in accepted_rows[request_index]
                if int(node_id) >= 0
            ]
            sampled = [
                int(token_id)
                for token_id in sampled_rows[request_index]
                if int(token_id) >= 0
            ]
            append_residual_tree_trace(
                self.residual_tree_trace_path,
                {
                    ...
                    "verification_mode": "greedy" if greedy else "stochastic",
                    "input_attention_group_indices": (
                        metadata.input_attention_group_indices
                    ),
                    "input_mamba_group_indices": (metadata.input_mamba_group_indices),
                    "nodes": [
                        {
                            "node_id": global_node_id - start,
                            "parent_node_id": (
                                -1
                                if int(parent_ids[global_node_id]) < 0
                                else int(parent_ids[global_node_id]) - start
                            ),
                            "token_id": int(node_tokens[global_node_id]),
                            "input_token_id": (
                                None
                                if input_tokens[global_node_id] is None
                                else int(input_tokens[global_node_id])
                            ),
                            "input_position": (
                                input_positions[global_node_id]
                                if input_positions[global_node_id] is not None
                                else None
                            ),
                            "input_attention_slots": (
                                input_attention_slots[global_node_id]
                                if input_attention_slots[global_node_id] is not None
                                else None
                            ),
                            "input_mamba_state_blocks": (
                                input_mamba_state_blocks[global_node_id]
                                if input_mamba_state_blocks[global_node_id] is not None
                                else None
                            ),
                            "depth": int(depths[global_node_id]),
                            "path_priority": float(priorities[global_node_id]),
                            "verifier_top1_token_id": int(targets[global_node_id]),
                            "verifier_top2_token_ids": (
                                [
                                    int(token_id)
                                    for token_id in top2_token_ids[global_node_id]
                                ]
                                if top2_token_ids is not None
                                else None
                            ),
                            "verifier_top2_logits": (
                                [float(logit) for logit in top2_logits[global_node_id]]
                                if top2_logits is not None
                                else None
                            ),
                            "verifier_top1_margin": (
                                float(
                                    top2_logits[global_node_id][0]
                                    - top2_logits[global_node_id][1]
                                )
                                if top2_logits is not None
                                else None
                            ),
                        }
                        for global_node_id in range(start, end)
                    ],
                    "accepted_node_ids": accepted,
                    "accepted_path_length": len(accepted),
                    "stopped_at_node_id": accepted[-1] if accepted else 0,
                    "sampled_token_ids": sampled,
                    "fallback_token_id": sampled[-1] if sampled else None,
                },
            )
    ...
    @staticmethod
    def apply_tree_logits_processors(
        logits: torch.Tensor,
        sampling_metadata: SamplingMetadata,
        metadata: TreeSpecDecodeMetadata,
    ) -> torch.Tensor:
        for processor in sampling_metadata.logitsprocs.non_argmax_invariant:
            if isinstance(processor, MinTokensLogitsProcessor):
                logits = processor.apply_with_tree_spec_decode(
                    logits,
                    metadata.node_request_indices,
                    metadata.node_depths,
                )
    ...
    @staticmethod
    def apply_tree_sampling_constraints(
        logits: torch.Tensor,
        sampling_metadata: SamplingMetadata,
        metadata: TreeSpecDecodeMetadata,
    ) -> torch.Tensor:
        ...
        node_request_indices = metadata.node_request_indices.to(
            device=logits.device, dtype=torch.long, non_blocking=True
        )
        if sampling_metadata.temperature is not None:
            temperature = sampling_metadata.temperature[node_request_indices]
        ...
        if sampling_metadata.top_k is not None:
            top_k = sampling_metadata.top_k[node_request_indices]
        top_p = None
        if sampling_metadata.top_p is not None:
            top_p = sampling_metadata.top_p[node_request_indices]
