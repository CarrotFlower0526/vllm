...
class MinTokensLogitsProcessor(LogitsProcessor):
    ...
    def apply_with_tree_spec_decode(
        self,
        logits: torch.Tensor,
        node_request_indices: torch.Tensor,
        node_depths: torch.Tensor,
    ) -> torch.Tensor:
        """Tree-spec-decode version of apply().

        Each tree node's target logits describe the next-token distribution
        after accepting the node's prefix. Stop tokens are masked only while
        ``len(output_token_ids) + node_depth < min_tokens`` for that request.
        """
        if not self.min_toks:
            return logits
        ...
        node_req_arr = node_request_indices.detach().cpu().numpy()
        node_depth_arr = node_depths.detach().cpu().numpy()
        all_rows: list[np.ndarray] = []
        all_toks: list[np.ndarray] = []

        for req_idx, min_tok, current_len, stop_toks in entries:
            rows = np.nonzero(
                (node_req_arr == req_idx) & (current_len + node_depth_arr < min_tok)
            )[0].astype(np.int64, copy=False)
            if rows.size == 0:
                continue
            n_stop = len(stop_toks)
            all_rows.append(np.repeat(rows, n_stop))
            all_toks.append(np.tile(stop_toks, rows.size))
