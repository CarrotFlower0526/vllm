...
class QwenGatedDeltaNetAttention(GatedDeltaNetAttention):
    ...
    def __init__(
        ...
        if speculative_config is not None and speculative_config.residual_tree:
            # Only residual-tree engines shadow this method. Ordinary no-spec
            # engines retain the original call path and generated kernels.
            self._ordinary_output_projection = self._output_projection
            self._output_projection = self._output_projection_tree_dispatch
            self._tree_output_projection_pending = False
    ...
    def _output_projection_tree_dispatch(
        self,
        core_attn_out: torch.Tensor,
        z: torch.Tensor,
        output: torch.Tensor,
        ...
        if not self._tree_output_projection_pending:
            self._ordinary_output_projection(core_attn_out, z, output, num_tokens)
            return
        self._tree_output_projection_pending = False
        ...
        core_attn_out = core_attn_out.reshape(-1, core_attn_out.shape[-1])
        z = z.reshape(-1, z.shape[-1])
        core_attn_out = rmsnorm_gated_tree_fn(
            core_attn_out,
            self.norm.weight,
            z,
            self.norm.eps,
    ...
    def _forward_core(
        ...
        if attn_metadata.tree_depth_token_indices is not None:
            self._tree_output_projection_pending = True
            self._forward_core_tree(
                mixed_qkv=mixed_qkv,
                b=b,
                a=a,
                core_attn_out=core_attn_out,
    ...
    def _forward_core_tree(
        ...
        token_depths = attn_metadata.tree_depth_token_indices
        parent_depths = attn_metadata.tree_depth_parent_state_indices
        child_depths = attn_metadata.tree_depth_child_state_indices
        state_index_pairs = attn_metadata.tree_depth_state_index_pairs
        source_columns = attn_metadata.tree_depth_source_columns
        destination_columns = attn_metadata.tree_depth_destination_columns
        if token_depths is None or parent_depths is None or child_depths is None:
            raise ValueError("tree GDN metadata is incomplete")


        # Keep manually constructed metadata compatible. The serving builder
        # precomputes these tensors once so all GDN layers reuse them.
        if state_index_pairs is None:
            state_index_pairs = tuple(
                torch.stack((parent, child), dim=1).contiguous()
                for parent, child in zip(parent_depths, child_depths, strict=True)
            )
        if source_columns is None:
            source_columns = tuple(
                torch.zeros_like(state, dtype=torch.int32) for state in parent_depths
            )
        if destination_columns is None:
            destination_columns = tuple(
                torch.ones_like(state, dtype=torch.int32) for state in child_depths
            )
        ...
        for depth, (
            ...
            num_rows = token_indices.numel()
            if num_rows == 0:
                continue

            depth_mixed_qkv = mixed_qkv.index_select(0, token_indices)
            depth_a = a.index_select(0, token_indices)
            depth_b = b.index_select(0, token_indices)

            if depth == 0:
                # The root's parent and child are the same cache slot.
                depth_mixed_qkv = causal_conv1d_update(
                    depth_mixed_qkv,
                    conv_state,
                    conv_weights,
                    self.conv1d.bias,
                    self.activation,
                    conv_state_indices=child_state_raw,
                    validate_data=False,
                )
            else:
                # Read column 0 (parent) and write column 1 (child) in the
                # causal-conv kernel. Siblings therefore share the same input
                # state without materializing per-child copies.
                depth_mixed_qkv = causal_conv1d_update(
                    depth_mixed_qkv,
                    conv_state,
                    conv_weights,
                    self.conv1d.bias,
                    self.activation,
                    conv_state_indices=state_pair,
                    block_idx_last_scheduled_token=destination_column,
                    initial_state_idx=source_column,
                    validate_data=False,
                )

            # Run the exact kernel used by ordinary one-token decode.  Reading
            # the parent slot and writing the child slot preserves independent
            # siblings, while consuming packed QKV directly avoids a D2D QKV
            # rearrangement and its launch/copy cost.
            ...
            fused_recurrent_gated_delta_rule_packed_decode(
                ...
                initial_state=ssm_state,
                out=depth_out,
                ssm_state_indices=parent_state_raw,
                final_state_indices=child_state_raw,
                use_qk_l2norm_in_kernel=True,
            )
