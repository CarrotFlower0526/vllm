...
@triton.jit
def rms_norm_gated_tree_fwd_kernel(
    X,
    Y,
    W,
    Z,
    ...
    """Tree-only RMSNormGated with ordinary-RPB1 arithmetic per row."""
    row_base = tl.program_id(0) * ROWS_PER_BLOCK
    cols = tl.arange(0, BLOCK_N)
    col_mask = cols[None, :] < N

    # Keep every row's instruction shape identical to the ordinary
    # layer_norm_fwd_kernel with ROWS_PER_BLOCK=1. Static unrolling retains
    # the adaptive one-launch grid without coupling the reduction layout to
    # the total number of tree rows.
    for row_offset in tl.static_range(0, ROWS_PER_BLOCK):
        rows = row_base + row_offset + tl.arange(0, 1)
        mask = (rows[:, None] < M) & col_mask
        x = tl.load(
        ...
        w = tl.load(W + cols[None, :], mask=col_mask, other=0.0).to(tl.float32)
        y = x * rstd[:, None] * w
        z = tl.load(
        ...
        if ACTIVATION == "swish" or ACTIVATION == "silu":
            ...
            y *= tl.sigmoid(z)
        tl.store(
            Y + rows[:, None] * stride_y_row + cols[None, :], y, mask=mask
        )


def rmsnorm_gated_tree_fn(
    x: torch.Tensor,
    weight: torch.Tensor,
    z: torch.Tensor,
    eps: float,
    ...
    """One-launch tree RMSNormGated matching ordinary one-row BF16 decode."""
    if x.ndim != 2 or z.shape != x.shape:
        raise ValueError(
            "tree RMSNormGated expects matching two-dimensional x/z tensors"
        )
    if x.stride(-1) != 1 or z.stride(-1) != 1 or weight.stride(-1) != 1:
        raise ValueError("tree RMSNormGated inputs must be contiguous by feature")
    if activation not in ("silu", "swish", "sigmoid"):
        raise ValueError(f"unsupported tree RMSNormGated activation: {activation}")

    num_rows, num_cols = x.shape
    if weight.shape != (num_cols,):
        raise ValueError(
            f"tree RMSNormGated weight must have shape ({num_cols},), "
            f"got {tuple(weight.shape)}"
        )
    output = torch.empty_like(x)
    rstd = torch.empty(num_rows, dtype=torch.float32, device=x.device)
    rows_per_block = calc_rows_per_block(num_rows, x.device)
    block_n = triton.next_power_of_2(num_cols)
    num_warps = min(max(block_n // 256, 1), 8)
    rms_norm_gated_tree_fwd_kernel[
        (cdiv(num_rows, rows_per_block),)
    ](
        x,
        output,
        weight,
