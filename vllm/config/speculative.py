...
class SpeculativeConfig:
    ...
    residual_tree: bool = False
    """Enable fixed-budget residual-head SpecInfer tree drafting.

    The draft budget is still ``num_speculative_tokens``. When enabled, the
    EAGLE proposer must expose residual-head proposal probabilities and a
    shared tree transition API; vLLM will not silently fall back to linear
    drafting. Each active residual head contributes exactly one token at every
    expanded tree state.
    """
    residual_tree_max_depth: int | None = Field(default=None, ge=1)
    """Maximum residual-tree depth. ``None`` allows depth up to the node budget."""
    residual_tree_tree_policy: Literal[
        "best_first", "breadth_first", "eagle3_dynamic"
    ] = "best_first"
    """How the fixed node budget is allocated. ``best_first`` uses the scorer
    globally; ``breadth_first`` completes each temporal level before expanding
    the next one and is the scorer-independent complete-level diagnostic.
    ``eagle3_dynamic`` uses the released width-10 beam expansion followed by
    global pruning to the configured node budget at maximum depth eight."""
    ...
    """Optional append-only JSONL path for per-step selection/verification traces."""
    residual_tree_active_heads: (
        list[Annotated[int, Field(strict=True, ge=0)]] | None
    ) = Field(default=None, min_length=1)
    """Residual head ids to use. When omitted, all heads exposed by the model
    are active."""
    residual_tree_candidate_selection: Literal[
        "head_top1",
        "distinct_head_top1",
    ...
    ``gated_distinct_head_top1`` switches to the conditional proposal only when
    the best novel token passes ``residual_tree_min_novel_probability_ratio``;
    otherwise it retains the repeated winner and original proposal.
    ``stock_top2`` is a dedicated non-residual control: one Stock EAGLE H1
    distribution contributes its two highest-logit distinct tokens at every
    expanded tree state. ``stock_top10_dynamic`` reproduces the released
    EAGLE-3 width-10 frontier for a depth-8 tree with up to 60 nodes, while
    ``stock_top9_dynamic`` is the matched width-9 control.
    ``hybrid_top9_h2_dynamic`` is a diagnostic width-matched arm: frozen H1
    contributes ranks one through nine and residual H2 contributes its best
    token after those nine target ids are removed.
    The ``hybrid_top8_h2_top2_dynamic`` through
    ``hybrid_top5_h2_top5_dynamic`` layouts are separate non-residual
    diagnostics: H1 contributes the named number of ranked candidates and H2
    fills the remaining width after those H1 target ids are removed.
    ``hybrid_union_top10_dynamic`` is a SpecInfer-style diagnostic: H1 and H2
    each contribute their native top ten, identical target tokens are merged,
    and the ten highest weighted unique candidates are retained."""
    residual_tree_min_novel_probability_ratio: float = Field(
        default=0.0, ge=0.0, le=1.0
    )
    """For gated distinct selection, replace a repeated winner only when the
    best novel token has at least this fraction of its proposal probability."""
    residual_tree_head_lambdas: list[float] | None = None
    """Per-head lambda weights estimated from residual coverable mass."""
    residual_tree_depth_head_lambdas: list[list[float]] | None = None
    """Optional candidate-depth by ordered-head lambda calibration matrix.

    Row zero scores candidate depth one. This only changes the existing CPU
    dynamic-tree priority calculation; proposal and verifier probabilities are
    unchanged.
    """
    residual_tree_adapter_config: str | None = None
    """Path to a shared residual-head EAGLE adapter JSON config."""
    residual_tree_adapter_checkpoint: str | None = None
    """Optional override for the residual-head adapter checkpoint path."""
    ...
    def compute_hash(self) -> str:
        ...
        factors.append(self.residual_tree)
        factors.append(self.residual_tree_adapter_config)
        factors.append(self.residual_tree_adapter_checkpoint)
        factors.append(self.residual_tree_head_lambdas)
        factors.append(self.residual_tree_depth_head_lambdas)
        ...
        if self.residual_tree:
            if self.method not in ("eagle", "eagle3", "mtp"):
                raise ValueError(
                    ...
                    f"drafting, got method={self.method!r}."
                )
            if self.parallel_drafting:
                raise ValueError(
                    "residual_tree requires autoregressive residual-head "
                    "tree transitions and is incompatible with parallel_drafting."
                )
            if self.residual_tree_head_lambdas is not None and not (
                self.residual_tree_head_lambdas
            ):
                raise ValueError("residual_tree_head_lambdas must not be empty")
            active_heads = self.residual_tree_active_heads
            if active_heads is not None:
                if any(
                    current <= previous
                    for previous, current in zip(active_heads, active_heads[1:])
                ):
                    raise ValueError(
                        "residual_tree_active_heads must be strictly increasing "
                        "and unique"
                    )
                if self.residual_tree_head_lambdas is not None and active_heads[
                    -1
                ] >= len(self.residual_tree_head_lambdas):
                    raise ValueError(
                        "residual_tree_active_heads contains an id without a "
                        "corresponding lambda"
                    )
            if (
                self.residual_tree_adapter_checkpoint is not None
                and self.residual_tree_adapter_config is None
            ):
                raise ValueError(
                    "residual_tree_adapter_checkpoint requires "
                    "residual_tree_adapter_config"
                )
            if (
                self.residual_tree_adapter_config is not None
                and self.method != "eagle3"
            ):
                raise ValueError(
                    "residual_tree_adapter_config currently supports only "
                    "method='eagle3'"
                )
            if self.residual_tree_candidate_selection == "stock_top2":
                if self.method != "eagle3":
                    raise ValueError(
                        "stock_top2 tree drafting requires method='eagle3'"
                    )
                stock_depth = int(self.residual_tree_max_depth or 0)
                maximum_binary_nodes = (1 << (stock_depth + 1)) - 2
                if (
                    self.num_speculative_tokens < 2
                    or self.num_speculative_tokens > maximum_binary_nodes
                ):
                    raise ValueError(
                        "stock_top2 budget must fit the configured binary-tree depth"
                    )
                forbidden_stock_tree_options = {
                    "residual_tree_adapter_config": self.residual_tree_adapter_config,
                    "residual_tree_adapter_checkpoint": (
                        self.residual_tree_adapter_checkpoint
                    ),
                    "residual_tree_head_lambdas": self.residual_tree_head_lambdas,
                    "residual_tree_depth_head_lambdas": (
                        self.residual_tree_depth_head_lambdas
                    ),
                    "residual_tree_active_heads": self.residual_tree_active_heads,
                    "residual_tree_head_prior_probabilities": (
                        self.residual_tree_head_prior_probabilities
                    ),
                    "residual_tree_greedy_scorer_path": (
                        self.residual_tree_greedy_scorer_path
                    ),
                    "residual_tree_trace_path": self.residual_tree_trace_path,
                }
                supplied = [
                    name
                    for name, value in forbidden_stock_tree_options.items()
                ...
                if self.draft_sample_method != "greedy":
                    raise ValueError("stock_top2 requires greedy draft sampling")
                if self.residual_tree_tree_policy != "best_first":
                    raise ValueError(
                ...
                candidate_selection = self.residual_tree_candidate_selection
                candidate_width = 9 if candidate_selection == "stock_top9_dynamic" else 10
                if self.method != "eagle3":
                    raise ValueError(
                        f"{candidate_selection} tree drafting requires method='eagle3'"
                    )
                if (
                    not 1 <= self.num_speculative_tokens <= 60
                    or self.residual_tree_max_depth != 8
                ):
                    raise ValueError(
                        f"{candidate_selection} requires D8 with a node budget "
                        "between 1 and 60"
                    )
                if self.residual_tree_scorer_top_k != candidate_width:
                    raise ValueError(
                        f"{candidate_selection} requires scorer_top_k="
                        f"{candidate_width}"
                    )
                forbidden_stock_tree_options = {
                    "residual_tree_adapter_config": self.residual_tree_adapter_config,
                ...
                if self.residual_tree_min_novel_probability_ratio != 0.0:
                    raise ValueError(
                        f"{candidate_selection} does not use residual novelty gating"
                    )
                if self.draft_sample_method != "greedy":
                    raise ValueError(
                        f"{candidate_selection} requires greedy draft sampling"
                    )
                if self.residual_tree_tree_policy != "eagle3_dynamic":
                    raise ValueError(
                        f"{candidate_selection} requires eagle3_dynamic policy"
                    )
                if not self.residual_tree_batch_drafting:
                    raise ValueError(
                        f"{candidate_selection} requires batched tree drafting"
                    )
            elif self.residual_tree_tree_policy == "eagle3_dynamic":
                if (
                    not 1 <= self.num_speculative_tokens <= 60
                    or self.residual_tree_max_depth != 8
                ):
                    raise ValueError(
                        "eagle3_dynamic residual trees require D8 with a node "
                ...
                if not self.residual_tree_batch_drafting:
                    raise ValueError(
                        "eagle3_dynamic residual trees require batched drafting"
                    )
                depth_head_lambdas = self.residual_tree_depth_head_lambdas
                if depth_head_lambdas is not None:
                    expected_heads = (
                        len(self.residual_tree_head_lambdas)
                        if self.residual_tree_head_lambdas is not None
                        else None
                    )
                    if (
                        len(depth_head_lambdas) != self.residual_tree_max_depth
                        or expected_heads is None
                        or any(
                            len(row) != expected_heads for row in depth_head_lambdas
                        )
                        or any(
                            not math.isfinite(float(value))
                            or float(value) <= 0.0
                            or float(value) > 1.0
                            for row in depth_head_lambdas
                            for value in row
                        )
                    ):
                        raise ValueError(
                            "residual_tree_depth_head_lambdas must be a finite "
                            "positive [max_depth, num_heads] matrix with values "
                            "at most one"
                        )
            if self.rejection_sample_method == "synthetic":
                raise ValueError(
                    "residual_tree uses SpecInfer tree verification and does "
            ...
            if self.residual_tree_scorer_mode == "head_prior":
                ...
                if probabilities is None:
                    raise ValueError(
                        "head_prior residual-tree scorer requires "
                        "residual_tree_head_prior_probabilities"
                    )
                expected_candidates = (
                    len(self.residual_tree_active_heads)
                    if self.residual_tree_active_heads is not None
                    else (
                        len(self.residual_tree_head_lambdas)
                        if self.residual_tree_head_lambdas is not None
                        else None
                ...
                if (
                    ...
                    and len(probabilities) != expected_candidates + 1
                ):
                    raise ValueError(
                        "head-prior probabilities must contain one value per "
                        "active candidate plus none"
                    )
                if (
                    not probabilities
                    or any(
                    ...
                    raise ValueError(
                        "head-prior probabilities must be finite, non-negative, "
                        "and sum to one"
                    )
            elif self.residual_tree_head_prior_probabilities is not None:
                raise ValueError(
                    "residual_tree_head_prior_probabilities is only valid with "
                    "the head_prior scorer"
                )
            if (
                self.residual_tree_depth_head_lambdas is not None
                and self.residual_tree_tree_policy != "eagle3_dynamic"
            ):
                raise ValueError(
                    "residual_tree_depth_head_lambdas requires eagle3_dynamic"
                )
            if (
                self.residual_tree_scorer_mode == "greedy_listwise"
                and self.residual_tree_greedy_scorer_path is None
            ):
                raise ValueError(
                    "greedy_listwise residual-tree scorer requires "
                    "residual_tree_greedy_scorer_path"
                )
            if (
                self.residual_tree_scorer_mode != "greedy_listwise"
                and self.residual_tree_greedy_scorer_path is not None
            ):
                raise ValueError(
            ...
            raise ValueError(
                f"{self.residual_tree_candidate_selection} candidates require "
                "residual_tree=True"
            )
