...
class VllmConfig:
    ...
    @staticmethod
    def _verify_residual_tree_execution_mode(
        speculative_config: SpeculativeConfig | None,
        model_config: ModelConfig | None,
    ) -> None:
        """Reject compiled target execution for residual-tree decoding."""
        if (
            speculative_config is None
            or not speculative_config.residual_tree
        ...
        if not model_config.enforce_eager:
            raise ValueError(
                "Residual-tree speculative decoding requires target model "
                "eager execution (`--enforce-eager` / "
                "`enforce_eager=True`). Compiled target execution is not "
                "supported because residual-tree GDN output routing depends "
                "on eager-only per-step state."
            )
    ...
    def __post_init__(self):
        ...
        self._verify_residual_tree_execution_mode(
        ...
        if self.scheduler_config.async_scheduling:
            ...
            if self.speculative_config is not None:
                if self.speculative_config.residual_tree:
                    raise ValueError(
                        "Async scheduling is not compatible with residual-tree "
            ...
            if (
                ...
                self.speculative_config is not None
                and self.speculative_config.residual_tree
            ):
                logger.warning_once(
                    "Async scheduling is not compatible with residual-tree "
